        ...
        <main class="mt-4">
            <table width="100%" style="border-bottom: 2px solid rgb(232, 232, 232)" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr>
                        @php
                        $total = $billType->bills->where('student_id', $data->student->id)->sum('amount');
                        $total_unpaid = $billType->bills->where('student_id', $data->student->id)->where('status',
                        'UNPAID')->sum('amount');
                        $subtotal += $total;
                        @endphp
                        ...
                        <td align="right">Rp. {{ number_format($total_unpaid, 0, ',', '.') }}</td>
                        <td align="right">
                            <span class="badge-secondary">{{ $total_unpaid > 0 ? 'Belum Lunas' : 'Lunas' }}</span>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
