@extends('layouts.master', ['title' => 'Edit Setting Mutasi'])
@section('content')
    ...
    <div class="toolbar" id="kt_toolbar">
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Edit Setting</h1>
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <li class="breadcrumb-item text-dark">Edit Setting</li>
                </ul>
            </div>
            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <h2>Form Edit Jalur Lanjut Unit</h2>
                    </div>
                </div>
                <div class="card-body py-4">
                    <form action="{{ route('unit-transfer-config.update', $unitTransferConfig->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8">
                                <select name="from_school_id" class="form-select form-select-solid" data-control="select2" required>
                                    ...
                                    @foreach($schools as $school)
                                        <option value="{{ $school->id }}" {{ (old('from_school_id') ?? $unitTransferConfig->from_school_id) == $school->id ? 'selected' : '' }}>{{ $school->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8">
                                <select name="to_school_id" id="to_school_id" class="form-select form-select-solid" data-control="select2" required>
                                    ...
                                    @foreach($schools as $school)
                                        <option value="{{ $school->id }}" {{ (old('to_school_id') ?? $unitTransferConfig->to_school_id) == $school->id ? 'selected' : '' }}>{{ $school->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-8">
                                <select name="to_classroom_id" id="to_classroom_id" class="form-select form-select-solid" data-control="select2" required>
                                    ...
                                    @foreach($classrooms as $classroom)
                                        <option value="{{ $classroom->id }}" {{ (old('to_classroom_id') ?? $unitTransferConfig->to_classroom_id) == $classroom->id ? 'selected' : '' }}>{{ $classroom->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-8">
                                <select name="bill_type_id" class="form-select form-select-solid" data-control="select2" required>
                                    ...
                                    @foreach($billTypes as $type)
                                        <option value="{{ $type->id }}" {{ (old('bill_type_id') ?? $unitTransferConfig->bill_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8">
                                <input type="text" name="amount" class="form-control form-control-solid input-money" placeholder="Contoh: 500.000" value="{{ old('amount') ? number_format(old('amount'), 0, ',', '.') : number_format($unitTransferConfig->amount, 0, ',', '.') }}" required />
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8 d-flex align-items-center">
                                <div class="form-check form-check-solid form-switch fv-row">
                                    <input class="form-check-input w-45px h-30px" type="checkbox" name="is_active" id="is_active" value="1" {{ (old('is_active') ?? $unitTransferConfig->is_active) ? 'checked' : '' }} />
