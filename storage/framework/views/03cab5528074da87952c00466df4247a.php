...
<script src="<?php echo e(url('https://cdnjs.cloudflare.com/ajax/libs/jquery.mask/1.14.16/jquery.mask.min.js')); ?>"></script>
...
<script>
    ...
    const translate = (input, output) => {
        if ($(input).val() != '') {
            axios.get('<?php echo e(route('translate')); ?>', {
                params: {
                    text: $(input).val(),
                }
...
<?php $__currentLoopData = ['success', 'error', 'warning', 'info']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
<?php if(session($message)): ?>
<script>
    Swal.fire({
                title: '<?php echo e(ucfirst($message)); ?>',
                text: "<?= session($message) ?>",
                icon: '<?php echo e($message); ?>',
...
</script><?php /**PATH F:\Antigravity\Projects\cahayatasbih\resources\views/layouts/partials/script.blade.php ENDPATH**/ ?>